...
var tdMap = {
    ...
    'distribute': function(act) {
        if ((act.now <= act.book_end) || (act.now >= act.start_time) || (act.stage == 1) || (act.seat_status != 2)) {
            return false;
        } else {
            return true;
        }
    ...
    'operation_links': function(act, key) {
        ...
        for (i in links) {
            ...
            // else if (i=="distribute" && operationMap[i](act)) {
            //     var string = "'"+links[i]+"'";
            //     result.push('<a href="javascript:void(0);" id="dis'+links[i]+'" onclick="disact('+string+')"><span class="glyphicon glyphicon-' + operations_icon[i] + '"></span> ' + operations_name[i] + '</a>');
            // }
            else {}
        }
...
// function disact(actid){
//     //alert(actid);
//     var i, len, curact;
//     for(i = 0, len = activities.length; i < len; ++i){
//         if(activities[i].operations.distribute == actid){
//             curact = activities[i];
//             break;
//         }
//     }
//     var content = '确认开始活动：<span style="color:red">'+curact.name+'</span>抽签？';
//     $('#modalcontent_dis').html(content);
//     $('#'+activities[i].delete).css("background-color","#FFE4C4");
//     $('#distributeid').val(activities[i].delete);
//     $('#disModal').modal({
//       keyboard: false,
//       backdrop:false
//     });
...
// function disConfirm(){
//     var disid = $('#distributeid').val();
//     //alert(delid);
//     var tmp  ="/distribute/";
//     $.post(tmp,{'activityId':disid}, function(ret) {
//         $('#'+disid).css("background-color","#FFF");
//         window.location.href="/list/"
//     });
// }

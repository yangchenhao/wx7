...
class ImageUploadForm(forms.Form):
    ...
    menu = forms.ImageField()

def upload_img(request):
    if request.method == 'POST':
        form = ImageUploadForm( request.POST, request.FILES )  # 有文件上传要传如两个字段
        if form.is_valid():
            m = Activity.objects.get(id=1)
            m.pic_img = form.cleaned_data['image']
            m.save()
...
def activity_list(request):
    ...
    for act in actmodels:
        act_status = act.stage
        act_book_endtime = act.book_end
        now = datetime.now()
        if(act_status == 0 and  act_book_endtime < now):
            act_status = 2
        dt = wrap_activity_dict(act)
        dt['activity_status']=act_status
        dt['now'] = datetime.now()
        print dt
        activities += [dt]
...
def activity_create(activity):
    preDict = dict()
    for k in ['name', 'key', 'description', 'place', 'seat_status', 'total_tickets']:
...
def activity_modify(activity):
    ...
    now = datetime.now()
    if nowact.status == 0:
        keylist = ['name', 'key', 'description', 'place', 'pic_img', 'seat_status', 'total_tickets']
        timelist = ['start_time', 'end_time', 'book_start', 'book_end']
    elif nowact.status == 1:
        if now >= nowact.start_time:
            keylist = ['description', 'pic_img']
            timelist = ['start_time', 'end_time']
        elif now >= nowact.book_start:
            keylist = ['description', 'place', 'pic_img']
            timelist = ['start_time', 'end_time', 'book_end']
        else:
            keylist = ['description', 'place', 'pic_img', 'seat_status', 'total_tickets']
            timelist = ['start_time', 'end_time', 'book_end']
    else:
        keylist = []
        timelist = []
...
def activity_distribute(request, actid):
    print actid
    activities = Activity.objects.filter(id=actid)
    curact = activities[0]
    if not doDistribute(curact):
        return HttpResponseRedirect(s_reverse_activity_list())
    curact.status = 1
    curact.stage = 1
    curact.save()
    doDistribute_post(curact)
    return HttpResponseRedirect(s_reverse_activity_list())

def doDistribute(act):
    seats = Seat.objects.filter(activity_id=act.id)
    available = {}
    if not seats.exists():
        return False
    for item in seats:
        available[item.area] = item.status
    available['Z'] = -1
    tickets = Ticket.objects.filter(activity=act, m_status=0)
    if not tickets.exists():
        return False

    for item in tickets:
        if (available[item.wish1]>0):
            item.seat = item.wish1
            available[item.wish1] -=1
            item.m_status=1
            item.save()
        elif (available[item.wish2]>0):
            item.seat = item.wish2
            available[item.wish2] -=1
            item.m_status=1
            item.save()
        elif (available[item.wish3]>0):
            item.seat = item.wish3
            available[item.wish3] -=1
            item.m_status=1
            item.save()
    for item in seats:
        item.status = available[item.area]
        item.save()
    return True

def doDistribute_post(act):
    seats = Seat.objects.filter(activity_id=act.id)
    if not seats.exists():
        return False
    tickets = Ticket.objects.filter(activity=act, status=0)
    if not tickets.exists():
        return False

    for item in tickets:
        if (available[item.wish1] == 'Z'):
            for seat in seats:
                if (seat.status > 0):
                    item.seat = seat.area
                    item.m_status = 1
...
def activity_post(request):
    ...
    try:
        if 'id' in post:
            ...
            form = ImageUploadForm( request.POST, request.FILES )  # 有文件上传要传如两个字段
            if form.is_valid():
                activity.pic_img = form.cleaned_data['pic']
                activity.menu_img = form.cleaned_data['menu']
                activity.save()
                activity.menu_name = str(activity.menu_img).split('/')[1]
                activity.pic_name = str(activity.pic_img).split('/')[1]
                activity.save()

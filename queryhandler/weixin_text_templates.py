...
def get_text_ticket_pic(ticket):
    if (ticket.activity.seat_status == 2):
        return ""
...
def get_text_exam_tickets(tickets, now):
    ...
    for ticket in tickets: 
        if (ticket.m_status == 0 and ticket.activity.stage == 0):
            temp = '正在等待结果的票：'
        elif (ticket.m_status == 0 and ticket.activity.stage == 1):
            temp = '没有抢到的票：'
        else:
            temp = '抢到的票：'
        temp += ticket.activity.name + ' ' + get_text_link(s_reverse_ticket_detail(ticket.unique_id), '电子票')
